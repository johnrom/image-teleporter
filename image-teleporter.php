...
function bml_whitelist_hosts( $allow_external, $host, $url ) {
	// if it's an external host, we'll check against the accepted URLs
	// needs get_option('bml_whitelisted_hosts');
	$whitelisted_hosts = array();

	if ( ! $allow_external ) {

		foreach ( $whitelisted_hosts as $accepted_host ) {

			if ( in_array( $host, $whitelisted_hosts ) ) {
				$allow_external = true;
			}
		}
	}
...
add_action('http_request_host_is_external', 'bml_whitelist_hosts', 30, 3 );
...
function bml_it_find_imgs( $post_id ) {
	...
	if ( $category_list = get_option('bml_it_catlist') ) {
		$post_in_category = false;
		$post_category_list = get_the_category( $post_id );

		foreach ( $post_category_list as $category ) {
			if ( in_array( $category->cat_ID, explode( ',', $category_list ) ) ) {
				$post_in_category = true;
			}
		}
	...
	$post_author_list = get_option( 'bml_it_authlist' );
	if ( $post_author_list && ! in_array( $post->post_author, explode( ',', $post_author_list ) ) ) {
		return;
	}
	...
	$replaced = false;
	$replace_src 		= get_option('bml_it_replacesrc');
	$replace_src_key 	= get_option('bml_it_custtagname');

	$processed 	= get_post_custom_values( $replace_src_key, $post_id );
	$content 	= $post->post_content;

	$post_images 		= bml_it_get_img_tags($post_id);

	for ( $i = 0; $i < count( $post_images ); $i++ ) {

		if ( ! $processed || ! in_array( $post_images[ $i ], $processed ) ) {
			$parseurl = parse_url( $post_images[ $i ] );
			...
			$image_id   = bml_it_sideload( strtok( $post_images[ $i ], '?'), $filename, $post_id );
			$image_url 	= wp_get_attachment_url( $image_id );

			if ( ! is_wp_error( $image_url ) ) {

				if ( $replace_src === 'custtag' ) {
					add_post_meta( $post_id, $replace_src_key, $post_images[ $i ], false );

				} else {
					$trans 		= preg_quote( $post_images[ $i ], "/" );
					$content 	= preg_replace('/(<img[^>]* src=[\'"]?)(' . $trans . ')/', '$1' . $image_url, $content );
					$replaced 	= true;
				}

				$processed[] = $post_images[ $i ];
				$bml_it_count++;
			}
		}
	}
		...
		foreach ( $tmp as $key => $value ) {
			...
			if ( '' != $value && $found ) {
				if ( is_array( $post_meta[$value] ) ) {
					$post_images = bml_it_get_img_links( $post_meta[$value] );
					...
					foreach ( $post_images as $img ) {
						...
						$image_id      = bml_it_sideload( $img, $filename, $meta_post_id );
						$new_post_images[] = wp_get_attachment_url( $image_id );
					...
					for ( $i=0; $i< count( $post_meta[$value] ); $i++ ) {
						$replace = str_replace( $post_images, $new_post_images, $post_meta[$value] );
...
function bml_it_loadimage( $url ) {
	...
	if ( function_exists("curl_init") ) {
		...
		if ('http://l.yimg.com/g/images/photo_unavailable.gif' === $info['url'] ) {
			$body = '';
		}
	...
	if ( $batch ) {
		if ( $offset ) {
			...
			$pp = new WP_Query( array(
				'post_type' => array_merge( array( 'post', 'page' ), $cpt_list ),
				'numberposts' => $batch,
				'offset' =>  $offset,
				'post_status' => 'publish'
			) );
